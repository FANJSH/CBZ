...
void MulticellBurner::ForwardCalculationADTS_toOWPC(Burnup &bu, int med_target, int ngrp, int *bgrp)
{
  ...
  // fuga addition
  // for neutron calculation in Simplified-Model
  vector< vector<GroupData1D> > mic_sigf_sm(mednum_fuel); // [medid][nucn]
  ...
  for(int i=0;i<mednum_fuel;i++){
    ...
    //   if(nuclide_info[j]!=0){
    //     if(nuclide_info[j]==1){
    //       mic_sigf_sm[i][j].put_imax(ngrp);
    //     };
    //     mic_sigc_sm[i][j].put_imax(ngrp);
    //     mic_sign2n_sm[i][j].put_imax(ngrp);
    //   };
    // };
  };
  ...
  for(int st=0;st<burn_step+1;st++){ 
    ...
    for(int i=0;i<mednum_fuel;i++){
      ...
      //opc.CalThermalScatteringMatrix(med[0],xslib,3.93);
      opc.CalThermalScatteringMatrix(med[0],xslib,4.048); // ESAB from MVP library      
    med_adts[i]=med[0]; // for predicdotr calculation in SM
      med[0].CalMacroFromMicro();
...
/*
  ...
  xsn2n_1g_p_sm[i].resize(nucn,0.);
      ...
      if(st==0){
        ...
        lat2.GetMedium(i).GetMacxs()=bmed.GetMacxs();
      };
    };
      ...
      for(int jj=0;jj<mednum_nonfuel;jj++){
        ...
        if(st==0){
          ...
          lat2.GetMedium(mednum_nonfuel+jj).GetMacxs()=bmed.GetMacxs();
        }
      }; 
  ...
  xsn2n_1g_c_sm[i].resize(nucn,0.);
	...
	lat.GetMedium(i).GetMacxs()=med[0].GetMacxs(); // fuga addition
      };
  ...
  //++++++++++++++++++++++++++++++++++++++++++++++
    ...
    cout<<"# Correction ...\n";
      ...
      for(int i=0;i<mednum_fuel;i++){
	    ...
	    if(nuclide_info[k]!=0){
	      if(nuclide_info[k]==1){
  	      if(xsf_1g_p_sm[i][k]>0.){
            xsf_1g[st][i][k]=xsf_1g_c_sm[i][k]*xsf_1g_p[st][i][k]/xsf_1g_p_sm[i][k]; //for neutron fission cross section
          }else{
            xsf_1g[st][i][k]=xsf_1g_c_sm[i][k];
          }
        };
	      if(xsc_1g_p_sm[i][k]>0.){
          xsc_1g[st][i][k]=xsc_1g_c_sm[i][k]*xsc_1g_p[st][i][k]/xsc_1g_p_sm[i][k]; // for neutron capture cross section
        }else{
          xsc_1g[st][i][k]=xsc_1g_c_sm[i][k];
        }
        if(xsn2n_1g_p_sm[i][k]>0.){
          xsn2n_1g[st][i][k]=xsn2n_1g_c_sm[i][k]*xsn2n_1g_p[st][i][k]/xsn2n_1g_p_sm[i][k]; // for n_2n reaction cross section	      
        }else{
          xsn2n_1g[st][i][k]=xsn2n_1g_c_sm[i][k];
        }
      };
...
void MulticellBurner::ForwardCalculationADTS_SamSys(Burnup &bu, int med_target, int sm_step, int ngrp, int *bgrp)
{
  ...
  // enter
    ...
    int sz=id.size();
    vector<string> rct={"c"}; // "f", "c", "n2n" or "ALL"
    int react=0;
  ...
  // -----
    ...
    for(int i=0;i<nucn;i++){
      int num_rct=0;
      if(nuclide_info[i]!=0){
        if(nuclide_info[i]==1 && ncl_f)num_rct++;
        if(ncl_c)num_rct++;
        if(ncl_n2n)num_rct++;
      };
      ttl+=num_rct;
    ...
    // fout_rr<<"# micro reaction rate both Original-Model and Simplified-Model\n# ";
    // for(int i=0;i<sz;i++){fout_rr<<prt_nuc_nam[i]<<" ";};
    // fout_rr<<"\n# ";
    // if(ncl_f)fout_rr<<"fission ";
    // if(ncl_c)fout_rr<<"capture ";
    // if(ncl_n2n)fout_rr<<"n2n ";
    // fout_rr<<"\n#-----------------------------------------------------------------\n";
  ...
  mic_sign2n_sm[i].resize(nucn);
    ...
    fout_tf<<flx_med[i].get_sum()<<" "<<flx_med_sm[i].get_sum()<<" ";
      ...
      for(int i=0;i<mednum_fuel;i++){
        string strid=IntToString(i);
        fout_om<<" ("+strid+")";
        for(int j=0;j<(10*(1+sz*react*2)-3-strid.size());j++){fout_om<<" ";};
      };
      fout_om<<endl;
      ...
      for(int i=0;i<group;i++){
        ...
        for(int j=0;j<mednum_fuel;j++){
          real flx_10=flx_med[j].get_dat(i)/letwid;
          fout_om<<flx_10<<" ";
          for(int k=0;k<nucn;k++){
            int nucid=med[0].GetNuclideInTurn(k).GetMatnum();
      ...
      for(int ll=0;ll<sz;ll++){
        ...
        if(nucid==id[ll]){
          if(ncl_f){
            if(nuclide_info[k]==1){
              fout_om<<mic_sigf[bstmp][j][k].get_dat(i)<<" "<<mic_sigf[bstmp][j][k].get_dat(i)*flx_10<<" ";
            };
          };
          if(ncl_c)fout_om<<mic_sigc[bstmp][j][k].get_dat(i)<<" "<<mic_sigc[bstmp][j][k].get_dat(i)*flx_10<<" ";
          if(ncl_n2n)fout_om<<mic_sign2n[bstmp][j][k].get_dat(i)<<" "<<mic_sign2n[bstmp][j][k].get_dat(i)*flx_10<<" ";
        };
      ...
      for(int i=0;i<mednum_fuel_sm;i++){
        string strid=IntToString(i);
        fout_sm<<" ("+strid+")";
        for(int j=0;j<(10*(1+sz*react*2)-3-strid.size());j++){fout_sm<<" ";};
      };
      fout_sm<<endl;
      ...
      for(int i=0;i<ngrp;i++){
        ...
        for(int j=0;j<mednum_fuel_sm;j++){
          real flx_10=flx_med_sm[j].get_dat(i)/letwid;
          fout_sm<<flx_10<<" ";
          // Medium bmed=med_adts[j].Cond(ngrp,bgrp,flx_med[j],flx_med[j],true);
          for(int k=0;k<nucn;k++){
            int nucid=bmed.GetNuclideInTurn(k).GetMatnum();
      ...
      for(int ll=0;ll<sz;ll++){
        ...
        if(nucid==id[ll]){
          if(ncl_f){
            if(nuclide_info[k]==1){
              fout_sm<<mic_sigf_sm[j][k].get_dat(i)<<" "<<mic_sigf_sm[j][k].get_dat(i)*flx_10<<" ";
            };
          };
          if(ncl_c)fout_sm<<mic_sigc_sm[j][k].get_dat(i)<<" "<<mic_sigc_sm[j][k].get_dat(i)*flx_10<<" ";
          if(ncl_n2n)fout_sm<<mic_sign2n_sm[j][k].get_dat(i)<<" "<<mic_sign2n_sm[j][k].get_dat(i)*flx_10<<" ";
        };
      ...
      // fout_rr<<"#\n# micro reaction rate\n#\n";
      // fout_rr<<"# burnup step   : "<<st<<endl;
      // fout_rr<<"# burnup[GWD/t] : "<<acburn[st]<<endl;
      // fout_rr<<"#\n";
      // fout_rr<<"# Energy   Med ID\n";
      // fout_rr<<"# [eV]    ";
      // for(int i=0;i<mednum_fuel_sm;i++){
      //   string strid=IntToString(i);
      //   fout_r<<" ("+strid+")";
      //   for(int j=0;j<(10*(sz*react*2)-3-strid.size());j++){fout_rr<<" ";};
      // };
      // fout_rr<<endl;
