...
class GeneralSystem{
 protected:
  ...
  vector<int> total_inner_iteration;
 ...
 public:
  ...
  void AddMedium(int id,Medium &inp);
  ...
  void CalPerturbTotalTerm(GeneralSystem *sec,bool *flag,real *val);
  real CalPerturbTotalTerm(GeneralSystem *sec,bool *flag,int g);
  real CalPerturbTotalTerm(GeneralSystem *sec,int m,int g);
  ...
  SensitivityData CalDirectTermPointReactionRateSigf(int meshid,int matid);
  ...
  real CalPointReactionRateRatio(int x,int matnum,enum xstype react,int matnum2,enum xstype react2);
  real CalPointReactionRateRatio(int x,int y,int matnum,enum xstype react,int matnum2,enum xstype react2);
  real CalPointReactionRateRatio(int x,int y,int z,int matnum,enum xstype react,int matnum2,enum xstype react2);
  void CalPowerXY(bool fission_source=false);
  void ShowFissionSourceForXYPlot(int z=0);
  void ShowIntegratedFluxForXYPlot(int gtop, int gend, int z=0);
  void ShowReactionRateForXYPlot(GroupData1D &wgt, int z=0);
  void CalAxialPowerDistribution(int x,int y);
  GroupData1D GetIntegratedFlux(int medid,int mom=0);
  GroupData1D GetIntegratedFluxPerVolume(int medid,int mom=0);
  GroupData1D GetIntegratedFluxParVolume(int medid,int mom=0){return GetIntegratedFluxPerVolume(medid,mom);};
  ...
  void CalFixedFissionSource(real k=1., int oit=0);
  real CalFixedSourceWithFission(real epsf,int itermax,bool high_speed_option);
  real CalFixedSourceWithFissionUpScat();
  // +++ For GPT calculation
  void CalGPT(real keff,int nuc,int *nuc_id,enum xstype *xs,bool *on_mesh,real rr,int itermax=20);
  void CalGPT(real keff,int nuc,int *nuc_id,enum xstype *xs,bool *on_mesh);
  void CalGPT(real keff,real eps_f=1e-4,int itermax=20);
  void CalGPTForPointMicroReactionRate(real keff,int mesh_id,int nuc_id,enum xstype xs,real rr,int itermax=20);
  ...
  void PutPL(int l);
  void PutNdim(int i);
  void AddTotalInnerIteration(int i){total_inner_iteration[i]++;};
  ...
  int GetTotalInnerIteration(int i){return total_inner_iteration[i];};
  int GetTotalInnerIterationSum();
