  ...
  */
  ...
  for(int st=0;st<burn_step+1;st++){
	...
	*/
	...
	//155のみ相関
        //rc_155=alpha_155*np_p_155+(r0_155-n0_155*alpha_155);
        rc_155=r0_155+(np_p_155-n0_155)*alpha_155;
	...
	*/
	...
	for(int j=0;j<nucn;j++){
	  ...
	  if(((n0-np)/n0>1.e-2)&&(nucid==641550||nucid==641570)){
	    ...
	    //real s_p=r0/sasuga; //predictor計算での微視的吸収断面積
	    //real s_c=rp/total_flux[st][0][i];
	    //real alpha_s=(s_c-s_p)/(np-n0);//dσ/dNの計算
	    ...
	    //real dt=burn_time[st]; // day 流石変更でコメントアウト
	    real dt=burn_time[st]*60*60*24; // second //流石変更
	    //real dt=neutron_fluence/sasuga; // second //流石変更
	    real a=0.00106012*dt+0.0146436;
	    real b=-0.0144283*dt+0.0882253;
	    
	    r0=r0*1.e-24;//rp
	    //r0=s_p*sasuga*1.e-24;
	    real np_p=n0*exp(-r0*dt);
	    alpha=alpha*1.e-24;
	    //alpha_s=alpha_s*1.e-24;
	    real rc=alpha*np_p+(r0-n0*alpha);
	    //real rc=alpha_s*sasuga*np_p+(r0-n0*alpha_s*sasuga);
	    ...
	    for(int k=0;k<time_mesh;k++){
	      np_r=n0_r*exp(-rp_r*dt_r);
	      rc_r=alpha*np_r+(r0-n0*alpha);
	      //rc_r=alpha_s*sasuga*np_r+(r0-n0*alpha_s*sasuga);
	      r_r=(rp_r+rc_r)/2;
	      n0_r=n0_r*exp(-r_r*dt_r);
	      rp_r=r_r;
	    };
	    ...
	    cout<<r0<<" "<<R_p<<" "<<rp<<" "<<omega<<" "<<R_reference<<endl;
	    //----ppc終わり
	    */
