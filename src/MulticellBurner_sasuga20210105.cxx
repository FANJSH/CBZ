...
void MulticellBurner::ForwardCalculation(Burnup &bu, int med_target, bool adjoint)
{
  ...
  */
  ...
  //  vector< vector <real> > n5_r5(305);
  //n5_r5.resize(mednum_fuel*2);
  real n5_r5[mednum_fuel*2][305];
  real n5_r7[mednum_fuel*2][305];
  real n7_r5[mednum_fuel*2][305];
  real n7_r7[mednum_fuel*2][305];
  ...
  char out155[100];
  char out157[100];
  sprintf(out155,"out155_step%d.dat",burn_step);
  sprintf(out157,"out157_step%d.dat",burn_step);
  ofstream f_5(out155);
  ofstream f_7(out157);
  f_5.setf(ios::scientific);
  f_7.setf(ios::scientific);
  */
  ...
  ifstream fin3;
  ifstream fin4;
  fin1.open("ref_N5_R5.dat");
  fin2.open("ref_N5_R7.dat");
  fin3.open("ref_N7_R5.dat");
  fin4.open("ref_N7_R7.dat");
  for(int jj=0;jj<305;jj++){
    fin1>> aaa >> aaa >> aaa;
    fin2>> aaa >> aaa >> aaa;
    fin3>> aaa >> aaa >> aaa;
    fin4>> aaa >> aaa >> aaa;
    for(int ii=0;ii<mednum_fuel*2;ii++){
      fin1>>n5_r5[ii][jj];
      fin2>>n5_r7[ii][jj];
      fin3>>n7_r5[ii][jj];
      fin4>>n7_r7[ii][jj];
  ...
  ofstream fout1("tmp1.dat");
  ofstream fout2("tmp2.dat");
  ofstream fout3("out_157.dat");
  ofstream fout4("out_155.dat");
  char output2[100];
  char output3[100];
  char output4[100];
  char output5[100];
  sprintf(output2,"n05_r05_qua.dat",burn_step);
  sprintf(output3,"n05_r07_qua.dat",burn_step);
  sprintf(output4,"np5_rc5_qua.dat",burn_step);
  sprintf(output5,"np5_rc7_qua.dat",burn_step);
  ofstream ff2(output2);
  ofstream ff3(output3);
  ofstream ff4(output4);
  ofstream ff5(output5);
  ff2.setf(ios::scientific);
  ff3.setf(ios::scientific);
  ff4.setf(ios::scientific);
  ff5.setf(ios::scientific);
 
  /*
  ofstream out0("out155_0.dat");
  ofstream out1("out155_1.dat");
  ofstream out2("out155_2.dat");
  ofstream out3("out155_3.dat");
  ofstream out4("out155_4.dat");
  ofstream out5("out155_5.dat");
  ofstream out6("out155_6.dat");
  ofstream out7("out155_7.dat");

  ofstream out00("out157_0.dat");
  ofstream out11("out157_1.dat");
  ofstream out22("out157_2.dat");
  ofstream out33("out157_3.dat");
  ofstream out44("out157_4.dat");
  ofstream out55("out157_5.dat");
  ofstream out66("out157_6.dat");
  ofstream out77("out157_7.dat");
  */
  ...
  for(int st=0;st<burn_step+1;st++){
	...
	for(int i=0;i<mednum_fuel;i++){
	  ...
	  if(st>0){
	    ...
	    real r0_5=xsc_1g_p[st][i][pos_gd155]*total_flux_p[st][0][i]; //rp
	    real r0_7=xsc_1g_p[st][i][pos_gd157]*total_flux_p[st][0][i]; //rp
	    real t_5=old_r0155[i]+(r0_5-old_r0155[i])/(n0_5-old_n0155[i])*(old_np155[i]-old_n0155[i]);   	    real t_7=old_r0157[i]+(r0_7-old_r0157[i])/(n0_5-old_n0155[i])*(old_np155[i]-old_n0155[i]);
	    if(st>1){
	      ...
	      //rr_gd5[i]=rr_gd5[i]-(R_old_5[i]-t_5)/al_5_be*al_5;
	      //rr_gd7[i]=rr_gd7[i]-(R_old_7[i]-t_7)/al_7_be*al_7;
	      cout<<"point  "<<(R_old_5[i]-t_5)<<" "<<rr_gd5[i]/R_old_5[i]<<" "<<(R_old_5[i]-t_5)/R_old_5[i]*rr_gd5[i]<<" "<<rr_gd5[i]<<endl;
	    }else{
	      //rr_gd5[i]=rr_gd5[i]-(R_old_5[i]-t_5)/R_old_5[i]*rr_gd5[i];
	      //rr_gd7[i]=rr_gd7[i]-(R_old_7[i]-t_7)/R_old_7[i]*rr_gd7[i];
	    };
	      rr_gd5[i]=rr_gd5[i]-(R_old_5[i]-t_5)/R_old_5[i]*rr_gd5[i];
	      rr_gd7[i]=rr_gd7[i]-(R_old_7[i]-t_7)/R_old_7[i]*rr_gd7[i];
	  };

	  xsc_1g[st][i][pos_gd155]=rr_gd5[i]/tmp;
	  xsc_1g[st][i][pos_gd157]=rr_gd7[i]/tmp;
	...
	if(st>0){
	  ...
	  if(st>1){
	    ...
	    a=time, b=time_be, c=time_be_2;//dR/dtで規格化する場合

	    X_5=old_n0155[i];
	    X_5=time_be;//dR/dt
	    real al_5_be=aa_5*(2*X_5-b-c)/(a-b)/(a-c)+bb_5*(2*X_5-a-c)/(b-a)/(b-c)+cc_5*(2*X_5-a-b)/(c-a)/(c-b);//前のステップでのdR/dNを計算
	    real al_7_be=aa_7*(2*X_5-b-c)/(a-b)/(a-c)+bb_7*(2*X_5-a-c)/(b-a)/(b-c)+cc_7*(2*X_5-a-b)/(c-a)/(c-b);
	    
	    X_5=n0_5;
	    X_5=time;//dR/dt
	    real al_5=aa_5*(2*X_5-b-c)/(a-b)/(a-c)+bb_5*(2*X_5-a-c)/(b-a)/(b-c)+cc_5*(2*X_5-a-b)/(c-a)/(c-b);//このステップのdR/dNを計算
	    real al_7=aa_7*(2*X_5-b-c)/(a-b)/(a-c)+bb_7*(2*X_5-a-c)/(b-a)/(b-c)+cc_7*(2*X_5-a-b)/(c-a)/(c-b);
	    
	    //xsc155=xsc_1g[st][i][pos_gd155]-(old_xscc_5[i]-t_5)/old_xscc_5[i]*xsc_1g[st][i][pos_gd155];
	    //xsc157=xsc_1g[st][i][pos_gd157]-(old_xscc_7[i]-t_7)/old_xscc_7[i]*xsc_1g[st][i][pos_gd157]; //反応率の大きさで規格化  ip補正
	      
	    //xsc155=xsc_1g[st][i][pos_gd155]-(old_xscc_5[i]-t_5)/al_5_be*al_5/burn_time_be*burn_time[st];
	    //xsc157=xsc_1g[st][i][pos_gd157]-(old_xscc_7[i]-t_7)/al_7_be*al_7/burn_time_be*burn_time[st]; //dR/dNで規格化 gamma補正
	    xsc155=xsc_1g[st][i][pos_gd155]-(old_xscc_5[i]-t_5)/burn_time_be*burn_time[st];
	    xsc157=xsc_1g[st][i][pos_gd157]-(old_xscc_7[i]-t_7)/burn_time_be*burn_time[st];
	  }else{
	    //xsc155=xsc_1g[st][i][pos_gd155]-(old_xscc_5[i]-t_5)/old_xscc_5[i]*xsc_1g[st][i][pos_gd155];
	    //xsc157=xsc_1g[st][i][pos_gd157]-(old_xscc_7[i]-t_7)/old_xscc_7[i]*xsc_1g[st][i][pos_gd157]; //反応率の大きさで規格化　ip補正
	    xsc155=xsc_1g[st][i][pos_gd155]-(old_xscc_5[i]-t_5)/burn_time_be*burn_time[st];
	    xsc157=xsc_1g[st][i][pos_gd157]-(old_xscc_7[i]-t_7)/burn_time_be*burn_time[st]; //dR/dNで規格化 gamma補正

	  };	    
	  xsc_1g[st][i][pos_gd155]=xsc155;
	  xsc_1g[st][i][pos_gd157]=xsc157;
	  //cout<<rcc155<<" "<<tmp_5<<" "<<rcc157<<"  "<<tmp_7<<endl;
	...
	if(owpc_corr){
	  ...
	  ff2<<fwd_nuc[st][0][i].get_dat(pos_gd155)<<" "<<xsc_1g_p[st][i][pos_gd155]*total_flux_p[st][0][i]<<" ";
	  ff3<<fwd_nuc[st][0][i].get_dat(pos_gd155)<<" "<<xsc_1g_p[st][i][pos_gd157]*total_flux_p[st][0][i]<<" ";
	  ff4<<fwd_nuc[st+1][0][i].get_dat(pos_gd155)<<" "<<xsc_1g[st][i][pos_gd155]*total_flux[st][0][i]<<" ";
	  ff5<<fwd_nuc[st+1][0][i].get_dat(pos_gd155)<<" "<<xsc_1g[st][i][pos_gd157]*total_flux[st][0][i]<<" ";
	  if(st>0){
	    ...
	    //modify rc---------------------
	    real m_rc_155,m_rc_157;
	    
	    real t_R_5=old_r0155[i]+(r0_155-old_r0155[i])/(n0_155-old_n0155[i])*(old_np155[i]-old_n0155[i]);	      
	    real t_R_7=old_r0157[i]+(r0_157-old_r0157[i])/(n0_155-old_n0155[i])*(old_np155[i]-old_n0155[i]);

	    real alpha_5_before=1;
	    real alpha_7_before=1;
	    real alpha_5=1;
	    real alpha_7=1;
	    
	    if(st>1){
	      real a=n0_155, b=old_n0155[i], c=old_n0155_2[i];
	      real aa_5=r0_155, bb_5=old_r0155[i], cc_5=old_r0155_2[i];
	      real aa_7=r0_157, bb_7=old_r0157[i], cc_7=old_r0157_2[i];
	      real X_5=old_np155[i];
	      t_R_5=aa_5*(X_5-b)*(X_5-c)/(a-b)/(a-c)+bb_5*(X_5-a)*(X_5-c)/(b-a)/(b-c)+cc_5*(X_5-a)*(X_5-b)/(c-a)/(c-b);
	      t_R_7=aa_7*(X_5-b)*(X_5-c)/(a-b)/(a-c)+bb_7*(X_5-a)*(X_5-c)/(b-a)/(b-c)+cc_7*(X_5-a)*(X_5-b)/(c-a)/(c-b);
	      
	    };
	        
	    real beta_5=(t_R_5-old_rc155[i])/t_R_5;
	    real beta_7=(t_R_7-old_rc157[i])/t_R_7;
	    ...
	    real X_5=np_p_155;
	    real X_7=np_p_157;
	    real rc_155=(aa_5*(X_5-b_5)*(X_5-c_5)/(a_5-b_5)/(a_5-c_5))+(bb_5*(X_5-a_5)*(X_5-c_5)/(b_5-a_5)/(b_5-c_5))+(cc_5*(X_5-a_5)*(X_5-b_5)/(c_5-a_5)/(c_5-b_5));
	    real rc_157=(aa_7*(X_5-b_5)*(X_5-c_5)/(a_5-b_5)/(a_5-c_5))+(bb_7*(X_5-a_5)*(X_5-c_5)/(b_5-a_5)/(b_5-c_5))+(cc_7*(X_5-a_5)*(X_5-b_5)/(c_5-a_5)/(c_5-b_5)); // Gd-155 correlation
	    //real rc_157=(aa_7*(X_7-b_7)*(X_7-c_7)/(a_7-b_7)/(a_7-c_7))+(bb_7*(X_7-a_7)*(X_7-c_7)/(b_7-a_7)/(b_7-c_7))+(cc_7*(X_7-a_7)*(X_7-b_7)/(c_7-a_7)/(c_7-b_7)); // Gd-157 correlation
	...
	for(int j=0;j<nucn;j++){
	  ...
	  if(nucid==641550||nucid==641570){
	    ...
	    /*
	    if(nucid==641550) f_5<<np<<" "<<xsc_1g[st][i][j]*total_flux[st][0][i]<<" "<<n0<<" "<<rr_gd5[i]<<" ";
	    if(nucid==641570) f_7<<np<<" "<<xsc_1g[st][i][j]*total_flux[st][0][i]<<" "<<n0<<" "<<rr_gd7[i]<<" ";
	    */
	      if((n0-np)/np>1e-2){
	  ...
	  //sasuga addition
	  /*
	  if(nucid==641550) fout4<<(n_next-np)<<" "<<(n_next-np)/np<<" ";
	  if(nucid==641570) fout3<<(n_next-np)<<" "<<(n_next-np)/np<<" ";
	  */
...
void MulticellBurner::ShowEigenvalue()
{
  ...
  //cout.precision(10);
  char output[100];
  sprintf(output,"qua_step%d.dat",burn_step);
  ...
  for(int i=0;i<burn_step+1;i++){
    ...
    cout<<"\n";
    if(i<10)fout<<" ";
    fout<<i<<" "<<acday[i]<<" "<<acburn[i]<<" "<<keff[i]<<" ";
    fout<<"\n";
  };
};
...
void MulticellBurner::ShowNumberDensityReactionRate(int nucid, int nucid2)
{
  ...
  cout<<"\n";
 char output[100];
  sprintf(output,"qua_step%d_nn.dat",burn_step);
 ofstream fout(output);
  ...
  for(int i=0;i<burn_step;i++){
    ...
    for(int j=0;j<mednum_fuel;j++){
      ...
      fout<<fwd_nuc[i][0][j].get_dat(target)<<" "<<fwd_nuc[i][0][j].get_dat(target2)<<" ";
    };
    cout<<"\n";
    fout<<"\n";
  };
