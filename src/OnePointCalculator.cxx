...
OnePointCalculator::OnePointCalculator()
{
  background_xs_printing=false;
  cw_correction_org=true;
};
...
{
  //cout<<"!!! NUC\n";
  //cout<<med.GetNuclideInTurn(0).GetMatnum()<<"\n";
  
  //background_xs_printing=true;
  ...
  bool cw_correction=cw_correction_org;
  ...
  */
	...
	if(((exist_rpara[j]&&den!=0.)||den>1e-6)&&ng!=-1){	    
	  ...
	  if(i>=sttgrp&&i<=endgrp){
	    ...
	    */
	    ...
	    cout.precision(10);
	    //if(iter==0)cout<<id<<" "<<i<<" "<<sig0c<<"\n";
	    //if(iter==0&&id==922380)cout<<sig0c<<"\n";
	    if(iter==0&&id==400000)cout<<sig0c<<"\n";	    	    
	    */
		...
		/*
		if(id==922360&&i==57){
		  cout<<"#  sig0 : "<<sig0<<" / rval : "<<rval<<"\n";
		};
		*/
		
		if(background_xs_printing){
		  bgxs[j][i]=sig0+r_sigt;
		  //bgxs[j][i]=sig0+r_sigt-sig0c*den_inv; // w/o heterogeneous correction (for use of Bell factor optimization)		  		   
		};
		...
		if(group==107&&id==922380&&i>=37&&i<=57){
		  f_calc[3]=-cw_factor[i-37];
                ...
                if(group==172&&id==922380&&i>=57&&i<=89){
	          f_calc[3]=-cw_factor_172[i-57]; // negative means use of different equation 
	        };
                // for plutonium-242 (2.38-3.06eV)
		if(group==107&&id==942420&&i==60){
  ...
  if(background_xs_printing){
    ...
    cout<<"#\n# Background cross section (group[0-]/upper energy/BG XS)\n#\n";
    for(int i=0;i<nucnum;i++){
      if(med.GetNuclideInTurn(i).GetGrp()!=-1){
      cout<<"#\n#    Nuclide : "<<med.GetNuclideInTurn(i).GetMatnum()<<"\n#\n";
      for(int g=0;g<group;g++){
	if(bgxs[i][g]>0.){
	  cout<<"   ";
	  WriteOut(g,4);
	  cout<<"  "<<med.GetEnband().get_dat(g)<<"  "<<bgxs[i][g]<<"\n";
	};
      };
      cout<<"\n\n";
      };
...
void OnePointCalculator::CalThermalScatteringMatrix(Medium &med,XSLibrary &xslib,real cutoff)
{
  // + Note in 2022/8/11 +
  //
  // Self-shielding treatment is commented out below.
  // We attempt to capture the self-shielding effect by using the self-shielding factor table,
  // but it is incorrect.
  // The temperature effect in the self-shielding factor table is totally different from
  // those in the S(a,b) data.
  // In other words, if the self-shielding treatment below is considered,
  // the temperature effect is "double-counted", and the results should become errorneous.
  // This causes large difference in medium macroscopic cross sections
  // between CBZLIB-based one and FRENDY-based one.
  
  // + Note in 2021/9/1 +
  //
  // Below the neutron energy of [cutoff], elastic scattering and total cross sections
  // are overwritten by the thermal scattering data.
  // In this procedure, "current-weighted" total cross section is replaced by "flux-weighted"
  // cross section.
  // At this moment, the reason of this treatment is unclear, but this remains.
  // If the [CalMacroFromMicro] method of the [Medium] class is carried out after this method,
  // "current-weighted" total cross section is re-calculated,
  // and transport cross section is also revised.
  ...
  for(int i=0;i<med.GetNucnum();i++){
	...
	// ... self-shielding effect consideration
	/*
	vector<real> f_el_org(grp);
	vector<real> f_er_org(grp);
	*/
	    ...
	    cout.precision(8);
            if(l==0){
	      f_el_org[g]=med.GetNuclideInTurn(i).GetMicxs().GetData2d(sigel).get_dat(g,g)/xslib.GetLibData(id).GetXSData().GetData2d(sigel).get_dat(g,g);
	      f_er_org[g]=f_el_org[g];
	      if(g!=grp-1)f_er_org[g]=med.GetNuclideInTurn(i).GetMicxs().GetData2d(sigel).get_dat(g,g+1)/xslib.GetLibData(id).GetXSData().GetData2d(sigel).get_dat(g,g+1);
	      //cout<<l<<" "<<g<<" "<<f_el_org<<" "<<f_er_org<<"\n";
              //cout<<med.GetNuclideInTurn(i).GetMicxs().GetData2d(sigel).get_dat(g,g)<<"\n";
	      //cout<<xslib.GetLibData(id).GetXSData().GetData2d(sigel).get_dat(g,g)<<"\n";
	    };
	    */
  ...
  for(int g=init_grp;g<grp;g++){
    ...
    if(pl_med>0){
      /*
      real tmp=st-ss1*0.33333333333333333;
      if(tmp<0.){
	cout<<"# Negative transport XS in group "<<g<<".\n";
      };
      */
