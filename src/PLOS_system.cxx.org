	...
	if(i!=-1){
	  ...
	  for(int nd=0;nd<Ndim;nd++){
	    ...
	    coefs[g][i]=sigr.get_dat(g)*vol;
  	    ...
  	    for(int nd=0;nd<Ndim;nd++){
	      bool ledge=false;
	      //if(nd==0&&x==xedgel[y])ledge=true;
	      //if(nd==1&&y==yedgel[x])ledge=true;
	      //if(nd==2&&z==0)ledge=true;
	      ...
	      bool redge=false;
	      //if(nd==0&&x==xedger[y])redge=true;
	      //if(nd==1&&y==yedger[x])redge=true;
	      //if(nd==2&&z==mi->GetZF()-1)redge=true;
...
real PLOSSystem::CalFluxOmega(int g,real epsif)
{
  ...
  for(int iter=0;iter<itmax;iter++){
	...
	for(int x=xel;x<=xer;x++){
	  real flt=Src[id];
          //if(y!=yedgel[x]){flt-=tmparray[idt++]*Fl[meshid[z][y-1][x]];};
          //if(y!=yedger[x]){flt-=tmparray[idt++]*Fl[meshid[z][y+1][x]];};
          if(y!=edge[1][x][z][0]){flt-=tmparray[idt++]*Fl[meshid[z][y-1][x]];};
          if(y!=edge[1][x][z][1]){flt-=tmparray[idt++]*Fl[meshid[z][y+1][x]];};
          //if(z!=0)   {flt-=tmparray[idt++]*Fl[meshid[z-1][y][x]];};
          //if(z!=zm-1){flt-=tmparray[idt++]*Fl[meshid[z+1][y][x]];};
		 ...
		 vector< vector< vector< vector<real> > > > &d1)
{
  ...
  for(int iter=0;iter<itmax;iter++){
	...
	for(int x=xel;x<=xer;x++){
	  int id=meshid[z][y][x];
	  real flt=Src[id];
          //if(y!=yedgel[x]){flt-=(coefy[g][id]-d1[z][y][x][1])*Fl[meshid[z][y-1][x]];}; 
          //if(y!=yedger[x]){
          ...
          //if(z!=0){flt-=(coefz[g][id]-d1[z][y][x][2])*Fl[meshid[z-1][y][x]];};
          //if(z!=zm-1){
...
void PLOSSystem::CalInnerCoef(int g, real *tmparray)
{
  ...
  for(int z=0;z<zm;z++){
    for(int y=0;y<ym;y++){
      ...
      int id=meshid[z][y][xel];
      for(int x=xel;x<=xer;x++){
        //if(y!=yedgel[x]){tmparray[idt++]=coefy[g][id];};
        //if(y!=yedger[x]){tmparray[idt++]=coefy[g][meshid[z][y+1][x]];};
        if(y!=edge[1][x][z][0]){tmparray[idt++]=coefy[g][id];};
        if(y!=edge[1][x][z][1]){tmparray[idt++]=coefy[g][meshid[z][y+1][x]];};
        //if(z!=0)        {tmparray[idt++]=coefz[g][id];};
        //if(z!=zm-1)     {tmparray[idt++]=coefz[g][meshid[z+1][y][x]];};
...
void PLOSSystem::DoCMFDAcceleration(real delk)
{
  ...
  */
  ...
  vector<GroupData1D> fc_dr(TotM);
  vector<GroupData1D> fc_dz(TotM);
  ...
  for(int i=0;i<TotM;i++){
    ...
    fc_dr[i].put_imax(ngrp);
    fc_dz[i].put_imax(ngrp);
    ...
    fc_dr[i]=mesh[i].GetMed()->GetMacxs().GetDr().Cond(flx,ngrp,bgrp);
    fc_dz[i]=mesh[i].GetMed()->GetMacxs().GetDz().Cond(flx,ngrp,bgrp);
  ...
  vector< vector<real> > cc_dr(CTotM);
  vector< vector<real> > cc_dz(CTotM);
  ...
  for(int i=0;i<CTotM;i++){
    ...
    cc_dr[i].resize(ngrp,0.);
    cc_dz[i].resize(ngrp,0.);
    ...
    for(int z2=0;z2<mi->GetCMeshF(2,z1);z2++){
      ...
      for(int y1=0;y1<yr;y1++){
		...
		for(int g=0;g<ngrp;g++){
	          ...
	          cc_dr[index][g]+=volflx*fc_dr[id].get_dat(g);
	          cc_dz[index][g]+=volflx*fc_dz[id].get_dat(g);
  ...
  for(int i=0;i<CTotM;i++){
    for(int g=0;g<ngrp;g++){
      ...
      minp.GetMacxs().GetKai().put_data(g,1.);
      minp.GetMacxs().GetD().put_data(g,cc_d[i][g]/tmp);
      minp.GetMacxs().GetDr().put_data(g,cc_dr[i][g]/tmp);
      minp.GetMacxs().GetDz().put_data(g,cc_dz[i][g]/tmp);
      for(int g2=g;g2<ngrp;g2++){
...
real PLOSSystem::CalEdgeCurVacuum(int m,int dir,int g,int flag)
{
  ...
  //return d/(2.1313*d+(mesh[m].GetLen(dir)*0.5)*0.4692)*-1.;
...
real PLOSSystem::CalPerturbScatteringTerm(PLOSSystem *sec,int g,bool *flag)
{
  real ret=0.;
...
real PLOSSystem::CalPerturbScatteringTerm(PLOSSystem *sec,int g1,int g2,bool *flag)
{
  real ret=0.;
  for(int m=0;m<TotM;m++){
    if(flag[m]){
...
/*
void PLOSSystem::CalPerturbN2NTerm(PLOSSystem *sec,bool *flag,real *val)
{
  GroupData1D dsign2n(grp);
  for(int i=0;i<grp;i++){val[i]=0.;};
  for(int m=0;m<TotM;m++){
    if(flag[m]){
      dsign2n=sec->GetMesh(m).GetMed()->GetMacxs().GetSign2n()
  	             -mesh[m].GetMed()->GetMacxs().GetSign2n();
      real vol=mesh[m].GetVolume();
      for(int g=0;g<grp;g++){
        val[g]+=mesh[m].GetFlux(0).get_dat(g)*dsign2n.get_dat(g)
       *sec->GetMesh(m).GetFlux(0).get_dat(g)*vol;
      };
    };
  };
};
...
real PLOSSystem::CalPerturbN2NTerm(PLOSSystem *sec,bool *flag,int g)
{
  real ret=0.;
  for(int m=0;m<TotM;m++){
    if(flag[m]){
