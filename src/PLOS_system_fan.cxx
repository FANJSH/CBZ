...
#define heat_conduction_calculation 1
...
real PLOSSystem::CalReactivitySP3(PLOSSystem *adj_p2,PLOSSystem *fwd_p0,PLOSSystem *fwd_p2,real kunp,real kp,bool pr)//*adj_p2 是一个指针，指向包含adjoint phi 2实例/对象的地址
{
  ...
  real ip00=CalPerturbDenominator(fwd_p0); //<phi+', F'phi'>
  ...
  for(int i=0;i<grp;i++){
    ...
    sct_p2[i]=adj_p2->CalPerturbScatteringTermDiffusion(fwd_p0,flag,i);//Cross-term
    ...
    // ... This term is added to Leakage-x component below
...
void PLOSSystem::PrintFluxDistribution(int zz)
{
  cout.setf(ios::scientific);
  cout.precision(4);
  int xx=mi.GetFMesh(0); //Return to FMesh[0], which is the total meshes number in x direction.
  int yy=mi.GetFMesh(1); //Return to FMesh[1], which is the total meshes number in y direction.
  for(int y=0;y<yy;y++){ // y-direction if has. 
    cout<<"# Mesh "<<y<<" in y-direction. "<<endl;
    for(int x=0;x<xx;x++){  //x-direction. 
    cout<<"# Mesh "<<x<<" in x-direction. "<<endl;
    cout<<"# Group      Flux"<<endl;
      for(int g=0;g<grp;g++){
        int id=meshid[zz][y][x];
        if(id==-1){cout<<" ";}
        else{
          real tmp=GetMesh(id).GetFlux().get_dat(g);
          if(tmp>0){
            cout<<" "<<g<<" "<<tmp<<"\n";
          }else{
            cout<<"---ERROR: Negative value of flux,  please check the flux calculation.---\n";
          };
        };
      };
    };
...
real PLOSSystem::CalSP3(PLOSSystem &sys2, bool rigorous_bc,real alpha)
{
    ...
    for(int g=0;g<grp;g++){
      ...
      for(int iit=0;iit<iit_max;iit++){
        ...
        for(int m=0;m<totm;m++){
          ...
          sys2.GetMesh(m).PutSrcin(-0.4*org); // part of source term in Eq2
        };
  ...
  //OSP3-forward calculation function
real PLOSSystem::CalOSP3(bool rigorous_bc)
{
  PLOSSystem sys2;
  return CalOSP3(sys2,rigorous_bc);
};

real PLOSSystem::CalOSP3(PLOSSystem &sys2, bool rigorous_bc) //SP3 equations set has two equations, consequently two instance of PLOS are needed. 
{
  if(!opt.Forward()){ //Check the adjoint calculation has been carried our or not. "opt.Forward()" returns to a bool parameter "forward" which initially false.
  ...
  sys2.Init(Ndim,grp,nmed);         //sys2 initial, for this 2nd PLOS instance. 
  if(!IsPrintTrue())sys2.NoPrint(); //IsPrintTrue() returns to a bool global variable "print" which is false in default.
  /*--Add medium ifo into sys2--*/
  for(int i=0;i<nmed;i++){
    sys2.AddMedium(med[i]);
    sys2.GetMed(i).GetMacxs().GetData2d(sigs).set_zero();
    // +++ modification for diffusion coefficient
    for(int g=0;g<grp;g++){
      real org=med[i].GetMacxs().GetData1d(d).get_dat(g);
      sys2.GetMed(i).GetMacxs().GetData1d(d).put_data(g,org*0.6);
    };
    /*--Maybe the following loop is not necessary--
    // +++ special treatment to use (phi0+2phi2) in RHS of the 2nd equation
    for(int g=0;g<grp;g++){
      real sigr0=GetMed(i).GetMacxs().GetData1d(sigr).get_dat(g);
      sys2.GetMed(i).GetMacxs().GetData1d(sigt).add_data(g,sigr0);
    };*/
  };
  ...
  for(int iter=0;iter<oit_max;iter++){
    for(int g=0;g<grp;g++){
      ...
      CalSrcMultiplySystem(g,fiss,pl);
      for(int m=0;m<totm;m++){
	      o_src[m]=mesh[m].GetSrcin();  // (fission source + scattering source) for Eq.(1)
        sr_v_ipi4[m]=mesh[m].GetMed()->GetMacxs().GetData1d(sigr).get_dat(g)*mesh[m].GetVolume()*INV_PI4; // sigma_r term for Eq.(1)
        sr_tmp2[m]=mesh[m].GetMed()->GetMacxs().GetData1d(sigt).get_dat(g)*mesh[m].GetVolume()*INV_PI4; // sigma_t term for Eq.(2) 
      };

      for(int iit=0;iit<iit_max;iit++){
        int iter_2=iter;
	      if(iit!=0)iter_2=1;
        // +++ phi0+2phi2 calculation
        for(int m=0;m<totm;m++){
          real org=o_src[m]; // (fission source + scattering source) of Eq1
          real fl2=sys2.GetMesh(m).GetFlux().get_dat(g);  // phi2
          real tmp=2*fl2*sr_v_ipi4[m];                    // 2*sigr*phi2
	        mesh[m].PutSrcin(org+tmp);                      // source term for (phi0+2phi2)
        };
        ...
        // +++ Eq.(2) calculation
        for(int m=0;m<totm;m++){
          real fl02_1=mesh[m].GetFlux().get_dat(g); // (phi0+2phi2)
          sys2.GetMesh(m).PutSrcin(fl02_1*sr_tmp2[m]); // sigt x (phi0+2phi2)
        };
        sys2.CalFluxGeneral(g,epsf_p2,iter_2); // (phi0+7/2 phi2) is obtained.
	      if(err<1e-5)break;
      };

      /*--sys contains (phi0+2phi2), sys2 contains (phi0+7/2*phi2)
      Obtain phi2 through computation--*/
      for(int m=0;m<totm;m++){
        real fl02_1=mesh[m].GetFlux().get_dat(g);         // phi0+2phi2
        real fl02_2=sys2.GetMesh(m).GetFlux().get_dat(g); // phi+7/2phi2
        real tmp=(2./3.)*(fl02_2-fl02_1);                 // phi2
        mesh[m].GetFlux().put_data(g,fl02_1-(2*tmp));       // phi0 is updated into sys
        sys2.GetMesh(m).GetFlux().put_data(g,tmp);// phi2 is updated into sys2
      };
    ...
    //
    // See the notebook in 2010/7/20
    /* -- not available for OSP3 right now
    if(rigorous_bc){
      real v21_40=21./40.;
      real v3_40=3./40.;
      real v3_8=3./8.;
        ...
        for(int y=0;y<mi.GetYF();y++){
          for(int x=0;x<mi.GetXF();x++){
              ...
              for(int g=0;g<grp;g++){
	              ...
	              real coefs2=sigr2.get_dat(g)*vol;
                real fl2=sys2.GetMesh(i).GetFlux().get_dat(g);
                real fl02=mesh[i].GetFlux().get_dat(g)+2*fl2;
                for(int nd=0;nd<Ndim;nd++){
		              real dc0=mesh[i].GetDif(g,difc[nd]);
                  real dc2=sys2.GetMesh(i).GetDif(g,difc[nd]);
                  real length=lenh[nd]; // half_length
...
real PLOSSystem::CalSP3Adjoint(PLOSSystem &sys2,bool rigorous_bc,real alpha)
{
  ...
  for(int iter=0;iter<oit_max;iter++){
    for(int g=grp-1;g>=0;g--){
      ...
      CalSrcMultiplySystem(g,fiss,pl);
      for(int m=0;m<totm;m++){
	    o_src[m]=mesh[m].GetSrcin(); // (fission source + scattering source) of (phi0+phi2) at RHS of 1st equation. 
      ...
      for(int iit=0;iit<iit_max;iit++){
        ...
        real err=CalFluxGeneral(g,epsf_p02,iter_2); // phi0 is obtained: 1st equation is solved in this line. 
	      //cout<<g<<" "<<iit<<" "<<err<<"\n";

        // +++ phi2 calculation
        for(int m=0;m<totm;m++){//this loop prepares the so-called source term for 2nd equation. 
          real fl0=mesh[m].GetFlux().get_dat(g); // solved phi0
        ...
        sys2.CalFluxGeneral(g,epsf_p2,iter_2); // -(phi2 - 28/27 phi0) is calculated.
          ...
          real fl0=mesh[m].GetFlux().get_dat(g); // read the solved phi0
          sys2.GetMesh(m).GetFlux().put_data(g,-fl02+28./27.*fl0); // phi2 is overwritten by obtained "fl02" term: the -(phi2 - 28/27 phi0).
	      };
