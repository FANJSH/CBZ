...
{
  if(geom!="Sphere"&&geom!="Cartesian")exit(0);
  sphere=false;
...
real SNRSystem::CalFluxSlab(int g,real epsif)
{
  ...
  for(int j=0;j<TotM;j++){
    ...
    for(int i=0;i<sn;i++){Src[ind]=srcin[i]; ind++;};
  };
  delete [] srcin;

  real *inifl=new real[sn];
  for(int i=0;i<sn;i++){inifl[i]=0.;};
  ...
  for(int it=0;it<itmax;it++){
	...
	real rhs=(mu-0.5*sigtvol[m])*afl+ss;
	real bfl=rhs/(mu+0.5*sigtvol[m]);
	real cfl=(afl+bfl)*0.5;
	if(bfl<0.){
	  bfl=0.;
	...
	// Step differencing
	/*
	real rhs=(ss+fabs(mu)*afl);
        real bfl=rhs/(fabs(mu)+sigtvol[m]);
	real cfl=bfl; 
	*/
...
real SNRSystem::CalReactivity(SNRSystem *sec,real kunp,real kp,vector<real> &rho,bool pr)
{
  ...
  if(pr)cout<<"# Grp : Yield  : Absorption : Scattering : Leakage\n";
  for(int i=0;i<grp;i++){
    yld[i]/=ip;
    abs[i]/=ip;
    real sl=0.;
    ...
    if(pr){
      ...
      cout<<en<<"  "<<yld[i]/leth<<"  "<<abs[i]/leth<<"  "<<sl/leth<<"  "<<le/leth<<"  "<<grpsum/leth<<"\n";
...
void SNRSystem::CalSensitivityFixedSource(SNRSystem *sec,int nucnum,int *nucid)
{
  ...
  real *absorg=new real[nmed];
  real *totorg=new real[nmed];
  real *sigs0org=new real[nmed*grp];
  real *sigs1org=new real[nmed*grp];
  real *sigs2org=new real[nmed*grp];
  real *sigs3org=new real[nmed*grp];
  real *sigs4org=new real[nmed*grp];
  real *sigs5org=new real[nmed*grp];

  real delta=1.;
  real PI4_INV=1./PI4;
  // Because transport theory-based perturbation results are multiplied by 4PI in CBG.
      ...
      for(int k=i;k<maxg;k++){
        for(int j=0;j<nmed;j++){
          if(med[j].ExistNuclide(nid)){
            ...
            totorg[j]=sec->GetMed(j).GetMacxs().GetSigt(0).get_dat(i);
            real micsigs=sec->GetMed(j).GetNuclide(nid).GetMicxs().GetMicSigel(0).get_dat(i,k);
            sigs0org[j]=sec->GetMed(j).GetMacxs().GetSigs(0).get_dat(i,k);
            sec->GetMed(j).GetMacxs().GetSigs(0).add_data(i,k,den*micsigs);
	    real st0=sec->GetMed(j).GetNuclide(nid).GetMicxs().GetMicSigel(0).get_sumx().get_dat(i);
            if(pl>0){
              sigs1org[j]=sec->GetMed(j).GetMacxs().GetSigs(1).get_dat(i,k);
	      real st1=sec->GetMed(j).GetNuclide(nid).GetMicxs().GetMicSigel(1).get_sumx().get_dat(i);
              real a1=st1/st0;
              sec->GetMed(j).GetMacxs().GetSigs(1).add_data(i,k,den*a1*micsigs);
	    };
	    if(pl>1){
              sigs2org[j]=sec->GetMed(j).GetMacxs().GetSigs(2).get_dat(i,k);
  	      real st2=sec->GetMed(j).GetNuclide(nid).GetMicxs().GetMicSigel(2).get_sumx().get_dat(i);
              real a2=st2/st0;
              sec->GetMed(j).GetMacxs().GetSigs(2).add_data(i,k,den*a2*micsigs);
	    };
	    if(pl>2){
              sigs3org[j]=sec->GetMed(j).GetMacxs().GetSigs(3).get_dat(i,k);
  	      real st3=sec->GetMed(j).GetNuclide(nid).GetMicxs().GetMicSigel(3).get_sumx().get_dat(i);
              real a3=st3/st0;
              sec->GetMed(j).GetMacxs().GetSigs(3).add_data(i,k,den*a3*micsigs);
	    };
	    if(pl>3){
              sigs4org[j]=sec->GetMed(j).GetMacxs().GetSigs(4).get_dat(i,k);
	      real st4=sec->GetMed(j).GetNuclide(nid).GetMicxs().GetMicSigel(4).get_sumx().get_dat(i);
              real a4=st4/st0;
              sec->GetMed(j).GetMacxs().GetSigs(4).add_data(i,k,den*a4*micsigs);
	    };
	    if(pl>4){
              sigs5org[j]=sec->GetMed(j).GetMacxs().GetSigs(5).get_dat(i,k);
  	      real st5=sec->GetMed(j).GetNuclide(nid).GetMicxs().GetMicSigel(5).get_sumx().get_dat(i);
              real a5=st5/st0;
              sec->GetMed(j).GetMacxs().GetSigs(5).add_data(i,k,den*a5*micsigs);
	    };
        ...
        for(int j=0;j<nmed;j++){
          if(med[j].ExistNuclide(nid)){
            sec->GetMed(j).GetMacxs().GetSigt(0).put_data(i,totorg[j]);
	    sec->GetMed(j).GetMacxs().GetSigs(0).put_data(i,k,sigs0org[j]);
	    if(pl>0)sec->GetMed(j).GetMacxs().GetSigs(1).put_data(i,k,sigs1org[j]);
	    if(pl>1)sec->GetMed(j).GetMacxs().GetSigs(2).put_data(i,k,sigs2org[j]);
	    if(pl>2)sec->GetMed(j).GetMacxs().GetSigs(3).put_data(i,k,sigs3org[j]);
	    if(pl>3)sec->GetMed(j).GetMacxs().GetSigs(4).put_data(i,k,sigs4org[j]);
	    if(pl>4)sec->GetMed(j).GetMacxs().GetSigs(5).put_data(i,k,sigs5org[j]);
	  };
	};
      };
    };
