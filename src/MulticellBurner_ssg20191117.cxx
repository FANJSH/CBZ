  ...
  */
  ...
  for(int st=0;st<burn_step+1;st++){
    ...
    */
	...
	real r_before_155;
	real r_before_157;
	  n_before_155=fwd_nuc[st][0][i].get_dat(pos_gd154);
	  r_before_155=rr_gd4[i];
	  n_before_157=fwd_nuc[st][0][i].get_dat(pos_gd156);
	  r_before_157=rr_gd6[i];
	...
	real rp_155=xsc_1g[bstmp][i][pos_gd155]*total_flux[st][0][i]; //rc
	real rp_157=xsc_1g[bstmp][i][pos_gd157]*total_flux[st][0][i]; //rc
	real n0_r_155;
	real n0_r_157;
	real alpha_155;
	real alpha_157;
	...
	alpha_155=(rp_155-r0_155)/(np_155-n0_155);
	alpha_157=(rp_157-r0_157)/(np_155-n0_155);
	// (Correlation to Gd-157 number density)
        //real alpha_155=(rp_155-r0_155)/(np_157-n0_157);
        //real alpha_157=(rp_157-r0_157)/(np_157-n0_157);
	//(Correlation to each isotopes number density)
	//real alpha_155=(rp_155-r0_155)/(np_155-n0_155);
	//real alpha_157=(rp_157-r0_157)/(np_157-n0_157);

	// (Correlation to Gd-155 number density)
	rc_155=r0_155+(np_p_155-n0_155)*alpha_155; // Rc in toy-problem
	rc_157=r0_157+(np_p_155-n0_155)*alpha_157;
        // (Correlation to Gd-157 number density)
	...
	for(int k=0;k<time_mesh;k++){
	  ...
	  real np_r_157=(n0_r_157-r_before_157/rp_r_157*n_before_157)*exp(-rp_r_157*1e-24*dt_r)+r_before_157/rp_r_157*n_before_157; // consider 154,156
          ...
          // (Correlation to Gd-155 number density)
	  rc_r_155=r0_155+(np_r_155-n0_155)*alpha_155;
	  rc_r_157=r0_157+(np_r_155-n0_155)*alpha_157;
          // (Correlation to Gd-157 number density)
          //rc_r_155=r0_155+(np_r_157-n0_157)*alpha_155;
	  //rc_r_157=r0_157+(np_r_157-n0_157)*alpha_157;
	  //(Correlation to each isotopes number density)
	  //rc_r_155=r0_155+(np_r_155-n0_155)*alpha_155;
          //rc_r_157=r0_157+(np_r_157-n0_157)*alpha_157;
	  ...
	  n0_r_155=(n0_r_155-r_before_155/rp_r_155*n_before_155)*exp(-r_r_155*1e-24*dt_r)+r_before_155/rp_r_155*n_before_155; // consider 154,156
	  n0_r_157=(n0_r_157-r_before_157/rp_r_157*n_before_157)*exp(-r_r_157*1e-24*dt_r)+r_before_157/rp_r_157*n_before_157; // consider 154,156
	...
	for(int j=0;j<nucn;j++){
	  ...
	  if(nucid==641550||nucid==641570){
	    ...
	    if((n0-np)/n0>1.e-1){
	      ...
	      real rp=xsc_1g[bstmp][i][j]*total_flux[st][0][i]; //Rc
	      
	      //consider 154,156--------
		real n_before;
		real r_before;
		if(nucid==641550){
		  n_before=fwd_nuc[st][0][i].get_dat(pos_gd154);
		  r_before=rr_gd4[i];
		}else if(nucid==641570){
		  n_before=fwd_nuc[st][0][i].get_dat(pos_gd156);
		  r_before=rr_gd6[i];
		}
		...
		real rp_155=xsc_1g[bstmp][i][pos_gd155]*total_flux[st][0][i]; //rc
		real rp_157=xsc_1g[bstmp][i][pos_gd157]*total_flux[st][0][i]; //rc
		real old_np155=fwd_nuc[st-1][0][i].get_dat(pos_gd155);
		real old_np157=fwd_nuc[st-1][0][i].get_dat(pos_gd157);
		real old_r0155=xsc_1g[st-1][i][pos_gd155]*total_flux[st-1][0][i];
		real old_r0157=xsc_1g[st-1][i][pos_gd157]*total_flux[st-1][0][i];
		real a,aa,b,bb,c,cc;
		if(nucid==641550){
		  a=n0_155;
		  b=np_155;
		...
		//real np_p=(n0-r_before/r0*n_before)*exp(-r0*1e-24*dt)+r_before/r0*n_before; // consider 154,156
		
		real X=np_p;
		rc=aa*(X-b)*(X-c)/(a-b)/(a-c)+bb*(X-a)*(X-c)/(b-a)/(b-c)+cc*(X-a)*(X-b)/(c-a)/(c-b);
		n0_r=n0;
		real rp_r=r0;
		for(int k=0;k<time_mesh;k++){
		  /*
		  if(st==1&&i==0){
		    if(nucid==641550){
		      std::cout<<std::setprecision(10)<<n0_r<<" "<<rp_r<<endl;
		    }
		  }
		  */
		  real np_r=n0_r*exp(-rp_r*1e-24*dt_r);
		  //real np_r=(n0_r-r_before/rp_r*n_before)*exp(-rp_r*1e-24*dt_r)+r_before/rp_r*n_before; //consider 154,156
		  X=np_r;
		  real rc_r=aa*(X-b)*(X-c)/(a-b)/(a-c)+bb*(X-a)*(X-c)/(b-a)/(b-c)+cc*(X-a)*(X-b)/(c-a)/(c-b);
		  real r_r=(rp_r+rc_r)/2;
		  n0_r=n0_r*exp(-r_r*1e-24*dt_r);
		  //n0_r=(n0_r-r_before/rp_r*n_before)*exp(-r_r*1e-24*dt_r)+r_before/rp_r*n_before; //consider 154,156
		  rp_r=rc_r;
		}
	      }else{
		...
		rc=alpha*np_p+(r0-n0*alpha); // Rc in toy-problem
...
void MulticellBurner::ShowNumberDensityReactionRate(int nucid, int nucid2)
{
  ...
  for(int i=0;i<burn_step;i++){
    cout<<i<<" "<<acday[i]<<" "<<acburn[i]<<" ";
    for(int j=0;j<mednum_fuel;j++){
      real tmp=xsc_1g[i][j][target2]*total_flux[i][0][j];
