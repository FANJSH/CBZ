...
void MulticellBurner::PutIGI(IrregularGeometryInformation &igi0,IrregularGeometryInformation &igi,bool reflective)
{
  ...
  sys.CalTrajectory(igi0,8,0.02,45.); // Default
  //sys.CalTrajectory(igi0,1,0.1,45.);

  // +++ TrajectorySet for neutron flux calculation
  if(reflective){
    sys_f.PutBoundaryCondition(Reflective);
    //sys_f.CalTrajectory(igi,128,0.01,360.);
    ...
    //sys_f.CalTrajectory(igi,2,0.08,45.);
    //sys_f.CalTrajectory(igi,1,0.1,45.);    
    // (for NEL2020_3x3)
...
void MulticellBurner::PutIGI_NEL2020_3x3(IrregularGeometryInformation &igi0,IrregularGeometryInformation &igi,bool reflective)
{
  totm=igi.GetRegion();
  fuel_r=igi0.GetCircle(igi0.GetIcir()-1).GetR(); // [cm]
  ...
  if(reflective){
    ...
    //sys_f2.CalTrajectory(igi,16,0.05,360.);
    sys_f2.CalTrajectory(igi,8,0.06,360.); // Default       
  }else{
    sys_f.PutBoundaryCondition(Periodic);
    sys_f.CalTrajectory(igi,8,0.02,45.); // Default    
    ...
    //sys_f2.CalTrajectory(igi,2,0.05,45.);
    sys_f2.CalTrajectory(igi,1,0.06,45.); // Default        
  };
  // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
};
...
void MulticellBurner::PutIGI_NEL2020(IrregularGeometryInformation &igi_33)
{
  ...
  sys_f2.CalTrajectory(igi_33,64,0.02,360.);  
};
...
void MulticellBurner::CalculationNEL2020_3x3(Burnup &bu, int med_target, int adts_step)
{
  PreCalculation(bu);
  ForwardCalculationNEL2020_3x3(bu,med_target,adts_step);      
};
...
void MulticellBurner::CalculationNEL2020(Burnup &bu, int med_target)
{
  ...
  ForwardCalculationNEL2020SasugaFinal(bu,med_target);
};
...
void MulticellBurner::ForwardCalculation(Burnup &bu, int med_target, bool adjoint)
{
  ...
  //bool owpc_corr=false; // If [false], the (log-averaging) PC is used.
  if(adjoint)owpc_corr=false;
  ...
  for(int st=0;st<burn_step+1;st++){
	...
	real n0_gd5=fwd_nuc[st][0][i].get_dat(pos_gd155);   // initial 
        real np_gd5=fwd_nuc[st+1][0][i].get_dat(pos_gd155); // predictor results      
	real n0=fwd_nuc[st][0][i].get_dat(pos_gd157);   // initial 
        real np=fwd_nuc[st+1][0][i].get_dat(pos_gd157); // predictor results
	bool r_corr=false;
        if((n0-np)/np>1e-2&&(n0_gd5-np_gd5)/np_gd5>1e-2)r_corr=true;
	...
	if(st>0){
	  ...
	  */
	  ...
	  if(r_corr&&factor<2.){ // if the burnup length is too different, the correction is NOT applied.
	...
	if(owpc_corr){
	  ...
	  if(st>0){ // ... quadratic model
	    ...
	    */
	    /*
	    cout<<"# R_ref155/rc_155/r0_155/n0_r_155/n0_155 : "<<R_reference_155<<" "<<rc_155<<" "<<r0_155<<" "<<n0_r_155<<" "<<n0_155<<"\n";
	    cout<<"# R_ref157/rc_157/r0_157/n0_r_157/n0_157 : "<<R_reference_157<<" "<<rc_157<<" "<<r0_157<<" "<<n0_r_157<<" "<<n0_157<<"\n";	    
	    */
	    
	    //if((0>omega_155)||(1<omega_155)) omega_155=0.5;
	    //if((0>omega_157)||(1<omega_157)) omega_157=0.5;
	    if(!(omega_155>0&&omega_155<1)) omega_155=0.5;
	    if(!(omega_157>0&&omega_157<1)) omega_157=0.5;	    
	  ...
	  if(nucid==641550||nucid==641570){
		...
		if(nucid==641550){
		  n_next=exp(log(np)*(1-omega_155)+log(nc)*omega_155);
	          cout<<"#   Omega_155 in AOWPC : "<<omega_155<<"\n";
		}else if(nucid==641570){
		  n_next=exp(log(np)*(1-omega_157)+log(nc)*omega_157);
	          cout<<"#   Omega_157 in AOWPC : "<<omega_157<<"\n";		  
    	        };
	      }else{
		/*
...
void MulticellBurner::ForwardCalculationNEL2020SasugaFinal(Burnup &bu, int med_target)
{
  // Pre-determined parameters/constants
...
void MulticellBurner::ForwardCalculationNEL2020_3x3(Burnup &bu, int med_target, int adts_step)
{
  if(input_flux_level&&med_target==-1){
    cout<<"# Error in MulticellBurner::ForwardCalculationNEL2020.\n";
    cout<<"# [med_target] should NOT be -1 if neutron flux level is posed.\n";
  ...
  vector<GroupData1D> pre_flux2;
  pre_flux2.resize(mednum);
	...
	medium_wise_flux_change[i]=(flx_2.get_sum()/vol_med[i])/flx_med_2[i].get_sum();
	  ...
	  for(int k=0;k<nucn;k++){
	    ...
	    if(i<8){
	      ...
	      if(nucid==641550||nucid==641570){
	        if(nucid==641550) cout<<"Gd-155 "<< xsc_1g[st][i][k]<<" "<<xsc_1g[st-1][i][k]*(xsc_1g_2[st][i][k]/xsc_1g_2[st-1][i][k])<<endl;
	        if(nucid==641570) cout<<"Gd-157 "<< xsc_1g[st][i][k]<<" "<<xsc_1g[st-1][i][k]*(xsc_1g_2[st][i][k]/xsc_1g_2[st-1][i][k])<<endl;
	        //xsf_1g[st][i][k]=xsf_1g[st-1][i][k]*(xsf_1g_2[st][i][k]/xsf_1g_2[st-1][i][k]); //for neutron fission cross section
	        xsc_1g[st][i][k]=xsc_1g[st-1][i][k]*(xsc_1g_2[st][i][k]/xsc_1g_2[st-1][i][k]);//for neutron capture cross section
	        //xsn2n_1g[st][i][k]=xsn2n_1g[st-1][i][k]*(xsn2n_1g_2[st][i][k]/xsn2n_1g_2[st-1][i][k]); // for n_2n reaction cross section
	      };
...
real MulticellBurner::EigenvalueCalculationSPH(Burnup &bu)
{
  GeneralOption opt;  
  ...
  int rr_abs_medid=-1;
  //int rr_abs_medid=11;  
  GroupData1D rr_abs_het=lat.GetIntegratedReactionRate(siga,rr_abs_medid);
  // ... absorption reaction rate of entire system

  vector<GroupData1D> rr_abs_het_r(mednum);
  for(int i=0;i<mednum;i++){
    rr_abs_het_r[i]=lat.GetIntegratedReactionRate(siga,i);
    // ... absorption reaction rate of each medium
  };

  GroupData1D rr_abs_het_nogd(group);
  rr_abs_het_nogd.set_zero();
  for(int i=8;i<mednum;i++){
    rr_abs_het_nogd=rr_abs_het_nogd+lat.GetIntegratedReactionRate(siga,i);
    // ... absorption reaction rate of non-Gd regions
  };

  GroupData1D flx_het(group);
  flx_het.set_zero();
  for(int i=0;i<mednum;i++){
    flx_het=flx_het+lat.GetIntegratedFlux(i);
    // ... neutron flux of entire system    
  };
  ...
  for(int g=0;g<group;g++){
    ...
    cout<<e0<<" ";
    for(int j=0;j<mednum;j++){
      real flx=lat.GetIntegratedFlux(j).get_dat(g)/lat.GetVolumeParMedium(j);
      cout<<flx/letwid<<" ";
    };
    cout<<"\n";
  };
  ...
  // ... SPH factor initialization
  vector< vector<real> > sph_factor(mednum);
  for(int i=0;i<mednum;i++){
    sph_factor[i].resize(group,1.);
  };

  // ... SPH iteration
  int sph_iter_max=30;
  vector<real> kinf_sph;
  vector<real> errmax_sph;
  vector<real> factor_errmax_sph;
  vector<int> errmax_medium_list;
  vector<int> errmax_group_list;

  vector< vector< vector<real> > > errsph(sph_iter_max);
  vector< vector< vector<real> > > sph(sph_iter_max); // This is added later to print out.
  for(int ii=0;ii<sph_iter_max;ii++){
    errsph[ii].resize(mednum);
    sph[ii].resize(mednum);    
    for(int i=0;i<mednum;i++){
      errsph[ii][i].resize(group);
      sph[ii][i].resize(group);      
    };

  // ... SPH-correction to macroscopic cross sections except for fission spectrum (chi)
  vector<Medium> med_tmp(mednum);
  for(int i=0;i<mednum;i++){
    med_tmp[i]=lat.GetMedium(i);
    for(int g=0;g<group;g++){
      med_tmp[i].GetMacxs().MultiplyExceptForChi(sph_factor[i][g],g);
    };
  };

  
  // ... Low-order model calculations

  MECSystem latc(group,mednum);
  //latc.PutTrajectorySet(&sys_f); // original ray-tracing condition
  latc.PutTrajectorySet(&sys_f2);  // low-order ray-tracing condition
  for(int jj=0;jj<mednum;jj++){
    latc.AddMedium(med_tmp[jj]);
  };
  latc.PutRegMed(region_medium);
  latc.PutGeneralOption(opt);
  latc.PutThermalIteration(3);
  latc.PutPL(0);
  latc.NoCMRAcceleration();
  latc.NoTransportApprox();
  // ! The above method [NoTransportApprox] is very important
  //   if [med_tmp] is used to define medium-wise macroscopic XS
  //   since the tranport correction has been already adopted inthe medium instances in [lat] 
  latc.Print();
  real keffc=latc.CalIgen();
  kinf_sph.push_back(keffc);

  GroupData1D rr_abs_hom=latc.GetIntegratedReactionRate(siga,rr_abs_medid);

  vector<GroupData1D> rr_abs_hom_r(mednum);
  for(int i=0;i<mednum;i++){
    rr_abs_hom_r[i]=latc.GetIntegratedReactionRate(siga,i);
  };

  GroupData1D rr_abs_hom_nogd(group);
  rr_abs_hom_nogd.set_zero();
  for(int i=8;i<mednum;i++){
    rr_abs_hom_nogd=rr_abs_hom_nogd+latc.GetIntegratedReactionRate(siga,i);
  };

  GroupData1D flx_hom(group);
  flx_hom.set_zero();
  for(int i=0;i<mednum;i++){
    flx_hom=flx_hom+latc.GetIntegratedFlux(i);
  };
  ...
  for(int i=0;i<mednum;i++){
    for(int g=0;g<group;g++){
      real flx_om=lat.GetIntegratedFlux(i).get_dat(g);
      real flx_sm=latc.GetIntegratedFlux(i).get_dat(g);
      //real tmp=flx_om/flx_sm;
      //real tmp=flx_om/flx_sm*rr_abs_hom.get_dat(g)/rr_abs_het.get_dat(g);
      //real tmp=flx_om/flx_sm*(rr_abs_hom.get_dat(g)-rr_abs_hom_r[i].get_dat(g))/(rr_abs_het.get_dat(g)-rr_abs_het_r[i].get_dat(g));
      real tmp=flx_om/flx_sm*(rr_abs_hom.get_dat(g)-rr_abs_hom_nogd.get_dat(g))/(rr_abs_het.get_dat(g)-rr_abs_het_nogd.get_dat(g));
      //real tmp=flx_om/flx_sm*(rr_abs_hom_nogd.get_dat(g))/(rr_abs_het_nogd.get_dat(g));      
      //real tmp=flx_om/flx_sm*(flx_hom.get_dat(g))/(flx_het.get_dat(g));
      real err=fabs(tmp-sph_factor[i][g])/sph_factor[i][g];
      errsph[ii][i][g]=err;
      sph[ii][i][g]=tmp;
      if(err>errmax){
	errmax=err;
  ...
  errmax_sph.push_back(errmax);
  factor_errmax_sph.push_back(factor);
  errmax_medium_list.push_back(errmax_medium);
  errmax_group_list.push_back(errmax_group);  
  ...
  cout.precision(6);
  cout<<"#\n";
  cout<<"# K-inf in the original condition : "<<keff<<"\n";
  cout<<"#\n";
  cout<<"# Iter       Max.Res.Error   at Max.Res.Err\n";
  cout<<"#      K-inf in SPH          SPH    Medium-ID    group\n";
  for(int i=0;i<sph_iter_max;i++){
    cout<<"# "<<i<<" "<<kinf_sph[i]<<" "<<errmax_sph[i]<<"      "<<factor_errmax_sph[i]<<"      "<<errmax_medium_list[i]<<"      "<<errmax_group_list[i]<<"\n";
  };
  cout<<"#\n";

#if 0
  cout<<"# Residual error in SPH factor during iteration\n";
  for(int g=0;g<group;g++){
    cout<<"# Group : "<<g<<"\n";
    for(int ii=0;ii<sph_iter_max;ii++){
      cout<<ii<<" ";
...
#if 0
  cout<<"# SPH factor during iteration\n";
  for(int g=0;g<group;g++){
    cout<<"# Group : "<<g<<"\n";
    for(int ii=0;ii<sph_iter_max;ii++){
      cout<<ii<<" ";
...
#if 0
  cout<<"# Maximum error per group\n";
  for(int ii=0;ii<sph_iter_max;ii++){
    cout<<ii<<" ";
    for(int g=0;g<group;g++){
      real errmax=0.;
      for(int jj=0;jj<mednum;jj++){
	if(fabs(errsph[ii][jj][g])>errmax)errmax=fabs(errsph[ii][jj][g]);
      };
      cout<<errmax<<" ";
    };
    cout<<"\n";
...
void MulticellBurner::SensitivityCalculation(Burnup &bu,int med_normalize, int num_med, int *med_target,int num,int *nucid_target_list,string filename)
{
  ...
  */
  ...
  for(int ii=0;ii<num;ii++){
    ...
    if(nucid_turn==-1){
      ...
      cout<<"# Target nuclide ["<<nucid_target<<"] for sensitivity calculation cannot be found.\n";
      exit(0);
    };
  ...
  */
  ...
  //
  //  implemented in 2022/9/6, but not tested.
  ...
  for(int i=0;i<nucn;i++){
	...
	for(int j=0;j<channel;j++){
	  ...
	  for(int k=0;k<burn_step;k++){
	    int sub_step=sub_step_list[k];
            for(int m=0;m<mednum_fuel;m++){
              real xs=mic_sigc[k][m][i].get_dat(j);

	      GroupData1D flx(group);
	      flx.set_zero();
  	      for(int l=0;l<totm;l++){
	        if(region_medium[l]==m){
	  	  flx=flx+volflx_mesh[k][l];
	        };
	      };
	      flx=flx*(1./vol_med[m]);
	      
              real factor=mic_sigc[k][m][i]*flx*1e-24;	    	      
	      for(int l=0;l<sub_step;l++){
	        real den=fwd_nuc[k][l][m].get_dat(i);
	        real val=0.;
	        ...
	        int pos=bu.SearchNuclide(id2);
	        if(pos!=-1){
	  	  val+=adj_nuc[k][l][m].get_dat(pos)*power_factor[k][l]*factor*rat*den;
	        };
	        time+=delt[k][l];
	        val*=delt[k][l];
	        sum+=val;
...
#if 0
  // ... Temporary part for non-convergence problem investigation
  int st=1-1;
  //int st=23;  
  for(int m=0;m<mednum_fuel;m++){
    lat_gpt.GetMed(m).GetMacxs().DataCopyPL(macxs[st][m],0); // [macxs] for corrector
    lat_gpt.GetMed(m).TransportApproximation();
  };
  lat_gpt.SetZeroScalarFlux();
  lat_gpt.SetZeroScatSrc();
  ifstream fint;
  fint.open("/home/roko-de-go/CBGCAL/PIE_Takahama/src_sf97_2",ios::in);
  if(fint.fail()){
    cout<<"# Failed to open the file [src_sf97_2]\n";
    exit(0);
  };
  for(int m=0;m<totm;m++){
    GroupData1D srctmp(group);
    for(int g=0;g<group;g++){
      int tmp;
      real tmp2;
      fint>>tmp;
      fint>>tmp;
      fint>>tmp2;
      srctmp.put_data(g,tmp2);
    };
    lat_gpt.PutIsotropicSourcePerUnitVolume(m,srctmp);
  };
  lat_gpt.CalGPT_MEC(keff[st],1e-4,100); // [keff[st]] for corrector
  fint.close();
  exit(0);
#endif  
...
void MulticellBurner::SensitivityCalculationDirect(Burnup &bu, int med_normalize, int matid, int mt, int target_num, int *nucid, string snsname,bool sigt_preserve)   // hard-coded for 3x3 problem in May, 2022
{
  PreCalculation(bu);
  ...
  if(mt!=102&&mt!=18){
    cout<<"# Error in MulticellBurner::SensitivityCalculationDirect.\n";
  ...
  */
  ...
  for(int gg=0;gg<group+2;gg++){
    ...
    */
    ...
    // (original, but limited for one-region number density)
    for(int i=0;i<target_num;i++){
      for(int j=0;j<nucn;j++){
	if(med[0].GetNuclideInTurn(j).GetMatnum()==nucid[i]){
	  ...
	  real volsum=0.;
          for(int k=0;k<8;k++){
            sum+=fwd_nuc[burn_step][0][k].get_dat(j)*vol_med[k];
            volsum+=vol_med[k];
	  };
          response[i][g]=sum/volsum;
  ...
  for(int i=0;i<target_num;i++){
    ...
    sns[i].WriteFile("./",snsname+"."+midt.Name(nucid[i])+".mednum8");
  };
...
void MulticellBurner::ShowNumberDensityHistory(int prt_nuc,string *prt_nuc_nam,Burnup &bu,string opt,bool sum_only)
{
  ...
  for(int i=0;i<burn_step+1;i++){
    ...
    for(int j=0;j<prt_nuc;j++){
      ...
      //if(iii<8)medsum[i][j]+=val*fuel_vol; // (for tentative Gd-pin calculation)     
      sum+=val;
    };
    cout<<"  "<<sum<<"\n";
  };
  ...
  for(int i=0;i<mednum_fuel;i++){
    sumvol+=vol_med[i];
    //if(i<8)sumvol+=vol_med[i]; // (for tentative Gd-pin calculation)
  };
...
vector<real> MulticellBurner::ShowVolumeAveragedNumberDensity(int prt_nuc, string *prt_nuc_nam, int num_med, int *med_target)
{
  vector<real> ret(prt_nuc);
  
  cout.setf(ios::scientific);
  cout.precision(10);
  cout<<"#\n";
  cout<<"# Volume-averaged number density\n";
  cout<<"#\n";
  for(int i=0;i<prt_nuc;i++){
    int nucid_target=midt.ID(prt_nuc_nam[i]);
    int nucid_turn=med[0].SearchNuclide(nucid_target);
    if(nucid_turn==-1){
      cout<<"# Error in MulticellBurner::ShowVolumeAveragedNumberDensity.\n";
      cout<<"# Target nuclide ["<<nucid_target<<"] for sensitivity calculation cannot be found.\n";
      exit(0);
    };
    ...
    for(int j=0;j<num_med;j++){
      int jj=med_target[j];
      sum+=fwd_nuc[burn_step][0][jj].get_dat(nucid_turn)*vol_med[jj];
      volsum+=vol_med[jj];
    };
    sum/=volsum;
    cout<<"# "<<prt_nuc_nam[i]<<" : "<<sum<<"\n";
    ret[i]=sum;
  };
...
void MulticellBurner::ShowCalculationCondition()
{
  cout<<"#\n";
  cout<<"# Mesh-medium correspodance\n";
  cout<<"#\n";
  cout<<"# Mesh-ID      Medium-ID    Volume\n";
  for(int m=0;m<totm;m++){
    cout<<"#     "<<m<<"     "<<region_medium[m]<<"     "<<sys_f.GetVolume(m)<<"\n";
  };
};
...
void MulticellBurner::CalculationADTS_toOWPC(Burnup &bu, int med_target, int ngrp, int *bgrp)
{
  ...
  //ForwardCalculationADTS_toOWPC_20220208(bu,med_target,ngrp,bgrp);  
};

void MulticellBurner::Calculation_newPC(Burnup &bu, int med_target, int ngrp, int *bgrp)
{
  PreCalculation(bu);
  // ForwardCalculationADTS_toOWPC(bu,med_target,ngrp,bgrp);
  ForwardCalculationAOWPCtoADTS(bu,med_target,ngrp,bgrp,false);
};


void MulticellBurner::ForwardCalculationAOWPCtoADTS(Burnup &bu, int med_target, int ngrp, int *bgrp, bool adjoint)
{

  bool owpc_corr=true; // If [false], the (log-averaging) PC is used.
  bool adts_corr=true; // If [false], no considering adts-model in predicro and corrector calculation
  bool cmpr_adts=false; // If [true], each predictor and corrector calculation, in both Original-Model and Simplified-Model
  ...
  vector< vector<GroupData1D> > mic_sign2n_c;
  // fuga addition
  vector< vector<GroupData1D> > mic_sigf_sm; // [medid][nucn]
  vector< vector<GroupData1D> > mic_sigc_sm;
  vector< vector<GroupData1D> > mic_sign2n_sm;
  vector< vector< vector<real> > > xsf_1g_p_sm; // [tmp][medid][nucn]
  vector< vector< vector<real> > > xsc_1g_p_sm;
  vector< vector< vector<real> > > xsn2n_1g_p_sm;
  vector< vector< vector<real> > > xsf_1g_c_sm;
  vector< vector< vector<real> > > xsc_1g_c_sm;
  vector< vector< vector<real> > > xsn2n_1g_c_sm;
  // ------------------
  vector<GroupData1D> flx_med_c;
  if(corrector_calc){
    //int tmp=1;
    ...
    if(adts_corr){
      xsf_1g_p_sm.resize(tmp);
      xsc_1g_p_sm.resize(tmp);
      xsn2n_1g_p_sm.resize(tmp);
      xsf_1g_c_sm.resize(tmp);
      xsc_1g_c_sm.resize(tmp);
      xsn2n_1g_c_sm.resize(tmp);
      for(int i=0;i<tmp;i++){
        xsf_1g_p_sm[i].resize(mednum_fuel);
        xsc_1g_p_sm[i].resize(mednum_fuel);
        xsn2n_1g_p_sm[i].resize(mednum_fuel);
        xsf_1g_c_sm[i].resize(mednum_fuel);
        xsc_1g_c_sm[i].resize(mednum_fuel);
        xsn2n_1g_c_sm[i].resize(mednum_fuel);
        for(int j=0;j<mednum_fuel;j++){
          xsf_1g_p_sm[i][j].resize(nucn,0.);
          xsc_1g_p_sm[i][j].resize(nucn,0.);
          xsn2n_1g_p_sm[i][j].resize(nucn,0.);
          xsf_1g_c_sm[i][j].resize(nucn,0.);
          xsc_1g_c_sm[i][j].resize(nucn,0.);
          xsn2n_1g_c_sm[i][j].resize(nucn,0.);
        };
      };

      mic_sigf_sm.resize(mednum_fuel);
      mic_sigc_sm.resize(mednum_fuel);
      mic_sign2n_sm.resize(mednum_fuel);
      for(int i=0;i<mednum_fuel;i++){
  ...
  // +++ Eigenvalue calculation
  MECSystem lat(group,mednum); // for Original-Model
  lat.PutTrajectorySet(&sys_f);  

  // MECSystem lat_sm(ngrp,mednum); // for Simplified-Model
  // lat_sm.PutTrajectorySet(&sys_f2);
/*
  if(adjoint){
  ...
  */
  ...
  for(int st=0;st<burn_step+1;st++){
	...
	med[0].GetFlux().copy(aveflx);
      ...
      */
      if(adts_corr)med_adts[i]=med[0]; // for predicdotr calculation in SM
      med[0].CalMacroFromMicro();
	...
	//total_flux[st][j].resize(mednum_fuel);
      ...
      if(corrector_calc){            
        ...
        // ... with group collapsing from details group to fewer-group ...
        cout<<"#\n# Predictor Calculation in Simplified-Model ...\n";
      ...
      // xsn2n_1g_p_sm[i].resize(nucn,0.);
          ...
          // if(st==0){
            lat_sm.AddMedium(bmed); 
            lat_sm.GetMedium(i).NuclideClear();
          // }else{
          //   lat_sm.GetMedium(i).GetMacxs()=bmed.GetMacxs();
          // };
        };
          ...
          for(int jj=0;jj<mednum_nonfuel;jj++){
            ...
            // if(st==0){
              lat_sm.AddMedium(bmed);
            // }else{
            //   lat_sm.GetMedium(mednum_nonfuel+jj).GetMacxs()=bmed.GetMacxs();
            // }
          }; 

          // +++ eigenvalue calculation
          lat_sm.PutRegMed(region_medium);
          lat_sm.PutGeneralOption(opt);
          lat_sm.PutThermalIteration(3);
          lat_sm.PutPL(0);
          lat_sm.NoCMRAcceleration();
          lat_sm.Print();

          real keff_dummy=lat_sm.CalIgen();
          
          for(int i=0;i<mednum;i++){
            GroupData1D flx_sm=lat_sm.GetIntegratedFlux(i);
      ...
      medium_wise_flux_change[i]=flx_med[i].get_sum()/(flx_sm.get_sum()/vol_med[i]);
            ...
            if(i<mednum_fuel){
              for(int j=0;j<nucn;j++){
                if(nuclide_info[j]!=0){
                  if(nuclide_info[j]==1){
                xsf_1g_p_sm[st][i][j]=mic_sigf_sm[i][j].Cond(flx_sm);
              };
            xsc_1g_p_sm[st][i][j]=mic_sigc_sm[i][j].Cond(flx_sm);
            xsn2n_1g_p_sm[st][i][j]=mic_sign2n_sm[i][j].Cond(flx_sm);
                };
          };
            };
          };
      ...
      // end predictor calculation in simplified model
      // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	if(adjoint){
      ...
      if(!adts_corr || cmpr_adts){
      cout<<"#\n# Corrector calculation in Original-Model ...\n";
	...
	if(adjoint){
      ...
      if(adts_corr){
            ...
            // to solve neutron transport equation for Simplified-Model ...
      ...
      // xsn2n_1g_c_sm[i].resize(nucn,0.);
            for(int k=0;k<nucn;k++){
              if(nuclide_info[k]!=0){
                ...
                // if(nuclide_info[k]==1){
                //   mic_sigf_sm[i][k].copy(med[0].GetNuclideInTurn(k).GetMicxs().GetData1d(sigf)); 
                // };
                // mic_sigc_sm[i][k].copy(med[0].GetNuclideInTurn(k).GetMicxs().GetData1d(sigc)); 
                // mic_sign2n_sm[i][k].copy(med[0].GetNuclideInTurn(k).GetMicxs().GetData1d(sign2n)); 
              };
            };
            ...
            // if(st==0){
              ...
              lat_sm.GetMedium(i).NuclideClear();
            // }else{
              // lat.GetMedium(i).GetMacxs()=bmed.GetMacxs();
            // }

      // lat_sm.GetMedium(i).GetMacxs()=bmed.GetMacxs(); // fuga addition
          };
          ...
          for(int jj=0;jj<mednum_nonfuel;jj++){
            ...
            // if(st==0){
              lat_sm.AddMedium(bmed);
            // }else{
              // lat.GetMedium(mednum_fuel+jj).GetMacxs()=bmed.GetMacxs();
            // }
            
            // lat_sm.GetMedium(mednum_fuel+jj).GetMacxs()=bmed.GetMacxs();
          };
          ...
          real keff_corr_dummy=lat_sm.CalIgen();


          for(int i=0;i<mednum;i++){
            GroupData1D flx_sm=lat_sm.GetIntegratedFlux(i);
            // if(!cmpr_adts)flx_med_c[i]=flx_sm*(1./vol_med[i]);
            // GroupData1D flx_sm=lat.GetIntegratedFlux(i);
            if(!cmpr_adts)flx_med_c[i]=flx_sm*(1./vol_med[i]);
            // +++ One-group cross section storing
            if(i<mednum_fuel){
              for(int j=0;j<nucn;j++){
                if(nuclide_info[j]!=0){
                  if(nuclide_info[j]==1)xsf_1g_c_sm[st][i][j]=mic_sigf_sm[i][j].Cond(flx_sm);
            xsc_1g_c_sm[st][i][j]=mic_sigc_sm[i][j].Cond(flx_sm);
            xsn2n_1g_c_sm[st][i][j]=mic_sign2n_sm[i][j].Cond(flx_sm);
          };
              };
      };
          };
      
    // end of Corrector Calculation in Simplified-Model ...
    // ++++++++++++++++++++++++++

      // ... Correcting One-group cross section and Total-flux ...
      ...
      if(!cmpr_adts){ 
        ...
        if(nuclide_info[k]!=0){
          if(nuclide_info[k]==1){
              xsf_1g[st][i][k]=xsf_1g_c_sm[st][i][k];
          };
          xsc_1g[st][i][k]=xsc_1g_c_sm[st][i][k];
          xsn2n_1g[st][i][k]=xsn2n_1g_c_sm[st][i][k];
        };
    #endif	  
        ...
        if(nuclide_info[k]!=0){
          if(nuclide_info[k]==1){
            if(xsf_1g_p_sm[st][i][k]>0.)xsf_1g[st][i][k]=xsf_1g_c_sm[st][i][k]*xsf_1g_p[st][i][k]/xsf_1g_p_sm[st][i][k]; //for neutron fission cross section
          };
          if(xsc_1g_p_sm[st][i][k]>0.)xsc_1g[st][i][k]=xsc_1g_c_sm[st][i][k]*xsc_1g_p[st][i][k]/xsc_1g_p_sm[st][i][k]; // for neutron capture cross section
          if(xsn2n_1g_p_sm[st][i][k]>0.)xsn2n_1g[st][i][k]=xsn2n_1g_c_sm[st][i][k]*xsn2n_1g_p[st][i][k]/xsn2n_1g_p_sm[st][i][k]; // for n_2n reaction cross section	      
        };
    #endif	  
        ...
        if(i<8){
          ...
          if(nuclide_info[k]!=0){
            if(nuclide_info[k]!=0){
              if(xsf_1g_p_sm[st][i][k]>0.)xsf_1g[st][i][k]=xsf_1g_c_sm[st][i][k]*xsf_1g_p[st][i][k]/xsf_1g_p_sm[st][i][k];
            }
            if(xsc_1g_p_sm[st][i][k]>0.)xsc_1g[st][i][k]=xsc_1g_c_sm[st][i][k]*xsc_1g_p[st][i][k]/xsc_1g_p_sm[st][i][k];//for neutron capture cross section
            if(xsn2n_1g_p_sm[st][i][k]>0.)xsn2n_1g[st][i][k]=xsn2n_1g_c_sm[st][i][k]*xsn2n_1g_p[i][k]/xsn2n_1g_p_sm[st][i][k]; // for n_2n reaction cross section
          };
        };
    #endif	 
        ...
        if(i<8){
          ...
          if(nucid==641550||nucid==641570){
            ...
            // xsf_1g[st][i][k]=xsf_1g[st-1][i][k]*(xsf_1g_2[st][i][k]/xsf_1g_2[st-1][i][k]); //for neutron fission cross section
            if(xsc_1g_p_sm[i][k]>0.)xsc_1g[st][i][k]=xsc_1g_p[st][i][k]*xsc_1g_c_sm[i][k]/xsc_1g_p_sm[i][k];//for neutron capture cross section
            // xsn2n_1g[st][i][k]=xsn2n_1g[st-1][i][k]*(xsn2n_1g_2[st][i][k]/xsn2n_1g_2[st-1][i][k]); // for n_2n reaction cross section
          };
        };
    #endif	  
      ...
      //for(int i=0;i<8;i++){ // Only for Gd-bearing rod
        ...
        // confirm newPC method 
        
        cout<<"# confirm equal to OM and SM ...\n";
        bool stop_corr=false;
        for(int i=0;i<mednum_fuel;i++){
          cout<<"# total-flux : "<<medium_wise_flux_change[i]<<endl;
          for(int k=0;k<nucn;k++){
            int nucid=med[0].GetNuclideInTurn(k).GetMatnum();          
            if(nuclide_info[k]!=0){
              if(nuclide_info[k]==1){
                if(xsf_1g_p[st][i][k]!=xsf_1g_p_sm[st][i][k]){
                  cout<<"# Error in Pridictor calclulation :: One-group fission cross section!\n";
                  cout<<"# st:"<<st<<" nuclide_id:"<<nucid<<"\n";
                  cout<<abs(1.-xsf_1g_p[st][i][k]/xsf_1g_p_sm[st][i][k])<<endl;
                  stop_corr=true;
                };
                if(xsf_1g[st][i][k]!=xsf_1g_c_sm[st][i][k]){
                  cout<<"# Error in Corrector calculation :: One-group fission cross section!\n";
                  cout<<"# st:"<<st<<" nuclide_id:"<<nucid<<endl;
                  cout<<abs(1.-xsf_1g[st][i][k]/xsf_1g_c_sm[st][i][k])<<endl;
                  stop_corr=true;
                }
              };
              if(xsc_1g_p[st][i][k]!=xsc_1g_c_sm[st][i][k]){
                cout<<"# Error in Predictor calculation :: One-group capture cross section!\n";
                cout<<"# st:"<<st<<" nuclid_id:"<<nucid<<endl;
                cout<<abs(1.-xsc_1g_p[st][i][k]/xsc_1g_p_sm[st][i][k])<<endl;
                stop_corr=true;
              }
              if(xsc_1g[st][i][k]!=xsc_1g_c_sm[st][i][k]){
                cout<<"# Error in  Corrector calculation :: One-group capture cross section!\n";
                cout<<"# st:"<<st<<" nuclide_id:"<<nucid<<" \n";
                cout<<abs(1.-xsc_1g[st][i][k]/xsc_1g_c_sm[st][i][k])<<endl;
                stop_corr=true;
              };
              if(xsn2n_1g[st][i][k]!=xsn2n_1g_c_sm[st][i][k]){
                cout<<"# Error in Predictor calculation :: One-group n2n cross section!\n";
                cout<<"# st:"<<st<<" Nuclide_id:"<<nucid<<"\n";
                cout<<abs(1.-xsn2n_1g[st][i][k]/xsn2n_1g_c_sm[st][i][k])<<endl;
                stop_corr=true;
              };
              if(xsn2n_1g_p[st][i][k]!=xsn2n_1g_c_sm[st][i][k]){
                cout<<"# Error in Corrector calculation :: One-group n2n cross section!\n";
                cout<<"# st:"<<st<<" Nuclide_id:"<<nucid<<endl;
                cout<<abs(1.-xsn2n_1g_p[st][i][k]/xsn2n_1g_c_sm[st][i][k])<<endl;
                stop_corr=true;
              }
            };
          };
    ...
    cout<<"# Correction ...\n";
      ...
      for(int i=0;i<mednum_fuel;i++){
	    ...
	    if(nuclide_info[k]!=0){
	      if(nuclide_info[k]==1){
             cout<<"# fission\n";
             cout<<xsf_1g_p[st][i][k]<<" "<<xsf_1g_p_sm[i][k]<<" "<<xsf_1g_c_sm[i][k]<<endl;
  	        if(xsf_1g_p_sm[i][k]>0.)xsf_1g[st][i][k]=xsf_1g_p[st][i][k]*xsf_1g_c_sm[i][k]/xsf_1g_p_sm[i][k]; //for neutron fission cross section
            // if(isfinite(xsf_1g[st][i][k]))cout<<"# error in fission.\n";exit(0);
        };
         cout<<"# capture\n"<<xsc_1g_p[st][i][k]<<" "<<xsc_1g_p_sm[i][k]<<" "<<xsc_1g_c_sm[i][k]<<endl;
	      if(xsc_1g_p_sm[i][k]>0.)xsc_1g[st][i][k]=xsc_1g_p[st][i][k]*xsc_1g_c_sm[i][k]/xsc_1g_p_sm[i][k]; // for neutron capture cross section
        // cout<<xsc_1g[st][i][k]<<endl;
        // if(isfinite(xsc_1g[st][i][k]))cout<<"# error in capture.\n";exit(0);
        // if(xsn2n_1g[st][i][k]>0.){
             cout<<"# n2n\n"<<xsn2n_1g_p[st][i][k]<<" "<<xsn2n_1g_p_sm[i][k]<<" "<<xsn2n_1g_c_sm[i][k]<<endl;
  	        if(xsn2n_1g_p_sm[i][k]>0.)xsn2n_1g[st][i][k]=xsn2n_1g_p[st][i][k]*xsn2n_1g_c_sm[i][k]/xsn2n_1g_p_sm[i][k]; // for n_2n reaction cross section
